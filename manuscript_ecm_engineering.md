...
High-DER distribution operation requires distribution management systems (DMS)
to triage many forecasted feeder states before committing expensive AC power
flow, OPF, or MPC studies. This paper addresses a system-integration question
rather than a new learning-method question: how should a calibrated
voltage-risk screen be embedded into a DMS so that release, watch, and
corrective-audit decisions remain auditable? VoltGuard-ECM specifies an
integration architecture, input data contract, queue semantics, exception
rules, algorithmic decision logic, rolling drift audit, and AC-audit handoff
for a screening layer placed between forecast ingestion and trusted corrective
tools. The paper deliberately does not reuse the core prediction, calibration,
and release metrics of the companion method study. Its contribution is the
engineering envelope that makes any calibrated screen deployable: input
validity checks, queue labels, operator override logic, rolling drift audit,
and mandatory fallback to AC analysis under stale telemetry, unseen topology,
or out-of-envelope DER forecasts. A seven-day Python prototype processes 1008
simulated operating cycles and demonstrates release/watch/corrective-audit
queue assignment, fallback routing, operator override logging, and drift-alarm
records. VoltGuard-ECM is therefore a DMS-facing governance layer for
voltage-risk screening, not a claim that a screen replaces AC feasibility
